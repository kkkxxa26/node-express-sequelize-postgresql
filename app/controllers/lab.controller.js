...
// exports.findAllHead = (req, res) => {  // .findAll()—получаем все записи, подходящие под условие
//   const title = req.query.title; // query - есть ли параметр title в строке запроса
//   var condition = title ? { title: { [Op.iLike]: `%${title}%` } } : null;
//   var user=req.user;

//   LabHead.findAll({ where: condition }) 
//     .then(async data => {
...
exports.updateHead = (req, res) => { 
  ...
  LabHead.update(req.body, { // обновляем запись(лекцию) в бд
...
exports.createBody = (req, res) => { 
  // Валидация request
  if (!req.body.title || !req.body.head_id) { // если в req.body !(нет) title || content
    res.status(400).send({ // то выводим статус 400(ошибка) с сообщением 
      message: "ниче нет" //само соообщение 
    });
    return;
...
// Сохраняем Lections в бд
  LabBody.create(item) // create в готовой базе создаем строчку 
    .then(data => {
      res.send(data);
    })
    .catch(err => {//иначе попадаем в .catch()
...
exports.findAllBody = (req, res) => { 
  const head_id = req.query.head_id; // query - есть ли параметр title в строке запроса
...
exports.updateBody = (req, res) => { 
  ...
  LabBody.update(req.body, { // обновляем запись(лекцию) в бд
    where: { id : id } // где id(по какому столбцу обновляем):id(значение с которым мы сверяеся)
  })
    .then(num => { // в данном параметре получаем количество обновленных строк в бд
      if (num == 1) { // if одна(n-количество) строка была успешно обновлена
...
exports.deleteBody = (req, res) => {
  ...
  LabBody.destroy({ // удаляем запись из бд по ID
    where: { id: Id }  // где id(по какому столбцу обновляем):id(значение с которым мы сверяеся)
  })
    .then(num => {
      if (num == 1) { // если запись была удалена 1
...
// Сохраняем Lections в бд
  LabAnswers.create(item) // create в готовой базе создаем строчку 
    .then(data => {
      res.send(data);
    })
    .catch(err => {//иначе попадаем в .catch()
...
exports.findAllAnswer = (req, res) => { 
  const body_id = req.query.body_id; // query - есть ли параметр title в строке запроса
...
exports.updateAnswer = (req, res) => { 
  ...
  LabAnswer.update(req.body, { // обновляем запись(лекцию) в бд
    where: { id : id } // где id(по какому столбцу обновляем):id(значение с которым мы сверяеся)
  })
    .then(num => { // в данном параметре получаем количество обновленных строк в бд
      if (num == 1) { // if одна(n-количество) строка была успешно обновлена
...
exports.deleteAnswer = (req, res) => {
  ...
  LabAnswer.destroy({ // удаляем запись из бд по ID
    where: { id: Id }  // где id(по какому столбцу обновляем):id(значение с которым мы сверяеся)
  })
    .then(num => {
      if (num == 1) { // если запись была удалена 1
